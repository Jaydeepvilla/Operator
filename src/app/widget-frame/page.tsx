    ...
    fetch("/api/widget/session", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          // Notify host script of session recovery using canonical event
          postToParent({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between p-space-4 border-b border-border/30 bg-background/25 backdrop-blur-md">
        ...
        <Button
          onClick={() => postToParent({ type: OPERATOR_WIDGET_EVENTS.TOGGLE })}
                      ...
                      <div className="space-y-space-2">
                        ...
                        {bookingDate && (
                          <div className="pt-space-2">
                            ...
                            {loadingSlots ? (
                              <div className="text-caption text-muted-foreground flex items-center gap-space-1">
                                <Loader2 className="h-3 w-3 animate-spin" /> Fetching slots...
                              </div>
