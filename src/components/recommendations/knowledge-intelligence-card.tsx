...
import { BrainCircuit, TrendingUp, TrendingDown, Minus, CheckCircle, AlertCircle } from "lucide-react";
import { Button } from "@/components/shared/button";
import Link from "next/link";

interface KnowledgeIntelligenceCardProps {
  quality: QualityScoreResult;
}

export function KnowledgeIntelligenceCard({ quality }: KnowledgeIntelligenceCardProps) {
  return (
    <Card className="border border-border-default radius-xl overflow-hidden flex flex-col h-full bg-gradient-to-br from-background to-blue-500/5 hover:border-blue-500/30 transition-colors">
      <CardHeader className="border-b border-border-default/50 pb-space-4">
        ...
        <p className="text-caption text-muted-foreground mt-space-1">{quality.coverage}% Content Coverage</p>
      </CardHeader>
